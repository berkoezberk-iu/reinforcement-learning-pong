...
class LazyFrames:
    def __init__(self, frames):
        """Ensure that common frames between observations are only stored once to optimize memory usage."""
        self._frames = frames
        self._out = None

    def _force(self):
        if self._out is None:
            self._out = np.stack(self._frames, axis=0)  # CORRECT: shape will be [4, 84, 84]
            self._frames = None
        return self._out
...
class LazyFrameStackObservation(FrameStackObservation):
...
class ReplayBuffer:
    ...
    def push(self, state, action, reward, next_state, done):
        ...
        self.buffer.append((np.asarray(state), action, reward, np.asarray(next_state), done))
...
def compute_loss(model, target_model, replay_buffer, batch_size, gamma):
    ...
    next_actions = model(next_state).max(1)[1]  # argmax over next actions from model
...
# === Training ===
def train(env, model, target_model, optimizer, replay_buffer):
    ...
    while episode < MAX_EPISODES:
        ...
        while not done:
            ...
            if len(replay_buffer) >= START_TRAINING:
                ...
                loss = compute_loss(model, target_model, replay_buffer, BATCH_SIZE, GAMMA)
                optimizer.zero_grad()
                loss.backward()
...
if __name__ == "__main__":
    ...
    env = LazyFrameStackObservation(env, STACK_SIZE)
    ...
    if (live):
        ...
        train(env, model, target_model, optimizer, replay)
